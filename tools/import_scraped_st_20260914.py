# -*- coding: utf-8 -*-
"""ST01–ST14 スターターをスクレイプログから CardData .asset へ取り込む（未登録のみ）。GDは対象外。"""
import json
import re
import shutil
import sys
...
LOG_PATH = SCRAPER_ROOT / "logs" / "scrape_20260914_163532.log"
...
FEATURE_SEED = {
    ...
    # --- ST 新規特徴 ---
    "G Generation": {"id": 71, "key": "G_Generation", "display": "G Generation"},
    "Attack": {"id": 72, "key": "Attack", "display": "Attack"},
    "Durability": {"id": 73, "key": "Durability", "display": "Durability"},
    "Support": {"id": 74, "key": "Support", "display": "Support"},
    "Innovade": {"id": 75, "key": "Innovade", "display": "イノベイド"},
    "Marine": {"id": 76, "key": "Marine", "display": "Marine"},
    "Long-Range Weapon": {"id": 77, "key": "Long_Range_Weapon", "display": "Long-Range Weapon"},
}
...
def parse_log_entries():
    ...
    skip_log = set(m.group(1) for m in re.finditer(r"\[skip\] (ST(?:0[1-9]|1[0-4])-\d{3})", log_text))
...
MonoBehaviour:
  ...
  imageName: {{fileID: 0}}
  image: {{fileID: 0}}
  imageAddress: {json.dumps(f"Data/Images/{image_name}", ensure_ascii=False)}
  version: {set_num}
  sourceType: 2
  productLine: 2
  boosterSet: 0
...
def apply_effects(created_gcgs, effect_map):
    """rewrite_gd02_05_effects のパーサで timedEffects を書き込む。"""
    sys.path.insert(0, str(TOOLS_DIR))
    import rewrite_gd02_05_effects as rewriter

    # ST 向け Feature ID を追加
    ...
    empty = 0
    errors = []
    for gcg in created_gcgs:
        paths = list(CARDS_DIR.glob(f"{gcg} *.asset"))
        if not paths:
            errors.append(f"{gcg}: asset not found")
            continue
        p = paths[0]
        text = p.read_text(encoding="utf-8")
        effect_text = effect_map.get(gcg, "-")
        tm = re.search(r"^  type: (\d+)$", text, re.M)
        card_type = int(tm.group(1)) if tm else 0
        try:
            blocks, is_blocker, repair_amount = rewriter.build_from_text(effect_text, card_type)
            is_repair = 1 if repair_amount else 0
            if not blocks:
                empty += 1
            new_text = rewriter.replace_block(text, blocks, is_blocker, is_repair, repair_amount)
            # キーワード DeployTurnAttack 等
            if re.search(r"<High-Maneuver", effect_text) or re.search(r"\[Deployed Unit\]", effect_text):
                pass
            if re.search(r"can attack during the turn it is deployed", effect_text, re.I):
                new_text = re.sub(r"  isDeployTurnAttack: \d+", "  isDeployTurnAttack: 1", new_text, count=1)
...
def load_existing_pilots_into_seed():
    """既存 PilotId アセットを PILOT_SEED に取り込み（リンク解決用）。"""
    for path in PILOT_DIR.glob("PilotId_*.asset"):
        text = path.read_text(encoding="utf-8", errors="replace")
        id_m = re.search(r"^  id: (\d+)$", text, re.M)
        key_m = re.search(r"^  pilotKey: (.+)$", text, re.M)
        disp_m = re.search(r'^  displayName: (?:"([^"]*)"|(.*))$', text, re.M)
        if not id_m or not key_m:
            continue
        seed = {
            "id": int(id_m.group(1)),
            "key": key_m.group(1).strip(),
            "display": (disp_m.group(1) if disp_m and disp_m.group(1) is not None else (disp_m.group(2) if disp_m else "")).strip(),
        }
        # key → 英語名（アンダースコア→空白）
        en_name = seed["key"].replace("_", " ")
        if en_name not in PILOT_SEED:
            PILOT_SEED[en_name] = seed
        if seed["display"] and seed["display"] not in PILOT_SEED:
            PILOT_SEED[seed["display"]] = seed


def main():
    entries, skip_log = parse_log_entries()
    effect_map = {e["ID"]: e["効果"] for e in entries}

    load_existing_pilots_into_seed()
    pilot_master = read_json(PILOT_MASTER)
    next_pilot_id = [max((p["id"] for p in pilot_master["pilots"]), default=122) + 1]

    # 表記ゆれ
    PILOT_SEED.setdefault("Shuji Itō", PILOT_SEED.get("Shuji Ito") or PILOT_SEED.get("Shuji Ito"))
    if "Shuji Ito" in PILOT_SEED:
        PILOT_SEED["Shuji Itō"] = PILOT_SEED["Shuji Ito"]
    if "M Quve" in PILOT_SEED and "M'Quve" not in PILOT_SEED:
        PILOT_SEED["M'Quve"] = PILOT_SEED["M Quve"]
    if "M_Quve" in {s["key"] for s in PILOT_SEED.values()}:
        for s in PILOT_SEED.values():
            if s["key"] == "M_Quve":
                PILOT_SEED["M'Quve"] = s
                break
    ...
    created_ids = []
    created_gcgs = []
    created_by_set = {i: 0 for i in range(1, 15)}
    skipped_existing = []
    errors = []
    features_added = sorted({s["id"] for s in FEATURE_SEED.values() if 71 <= s["id"] <= 77})
    new_pilot_floor = next_pilot_id[0]
    # ensure_pilot_seed が進めた後の「新規」は floor 以上ではなく、開始時の floor
    # → 上で next_pilot_id を進めた後なので、開始値を保存し直す
    # 実際の新規は sync 後に id>=123 かつ今回追加されたもの
    ...
    effect_updated, effect_empty, effect_errors = apply_effects(created_gcgs, effect_map)
    errors.extend(effect_errors)

    skip_missing_data = [s for s in skip_log if s not in existing_gcg and s not in created_gcgs]
    pilots_added = sorted({s["id"] for s in PILOT_SEED.values() if s["id"] >= 123})
    ...
    print(f"effect_updated={effect_updated} effect_empty={effect_empty}")
    if errors:
        print("errors:")
